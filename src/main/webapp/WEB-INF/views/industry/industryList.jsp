...
function industryAddFun() {
        let id = "";
        let checks = $('#industryDataGrid').datagrid('getChecked');
    ...
    parent.$.modalDialog({
        ...
        buttons : [ {
            ...
            handler : function() {
                ...
                let f = parent.$.modalDialog.handler.find('#industryEditForm');
                f.submit();
            }
        } ]
    });
...
function industryEditFun(id) {
     if (id == undefined) {
            let rows = industryDataGrid.datagrid('getSelections');
 ...
 * 永久删除
 */
 function industryDeleteForever(id) {
     let tip="";
        if (id == undefined) {
            id="";
            let rows = industryDataGrid.datagrid('getSelections');
     ...
     if(id!=undefined && id!=null && id!='' ){
        parent.$.messager.confirm('询问', '您是否要永久删除当前数据？'+tip, function(b) {
            if (b) {
                progressLoad();
                $.post('${path}/industry/deleteForever', {
                    ids : id
                }, function(result) {
                    if (result.success) {
                        parent.$.messager.alert('提示', result.msg, 'info');
...
<div id="industryToolbar" style="display: none;">
    ...
    <shiro:hasPermission name="/industry/delete">
        <a onclick="industryDeleteFun();" href="javascript:void(0);" class="easyui-linkbutton" data-options="plain:true,iconCls:'glyphicon-remove  icon-yellow'">隐藏</a>
        <shiro:hasRole name="超级管理员">
            <a onclick="industryDeleteForever()" href="javascript:void(0);" class="easyui-linkbutton" data-options="plain:true,iconCls:'glyphicon-remove  icon-red'">永久删除</a>
        </shiro:hasRole>
    </shiro:hasPermission>
    <shiro:hasPermission name="/industry/add">
