...
function photographerAddFun() {
        let id = "";
        let checks = $('#photographerDataGrid').datagrid('getChecked');
    ...
    parent.$.modalDialog({
        ...
        buttons : [ {
            ...
            handler : function() {
                ...
                let f = parent.$.modalDialog.handler.find('#photographerEditForm');
                f.submit();
            }
        } ]
    });
...
function photographerEditFun(id) {
     if (id == undefined) {
            let rows = photographerDataGrid.datagrid('getSelections');
 ...
 * 永久删除
 */
 function photographerDeleteForever(id) {
     let tip="";
        if (id == undefined) {
            id="";
            let rows = photographerDataGrid.datagrid('getSelections');
     ...
     if(id!=undefined && id!=null && id!='' ){
        parent.$.messager.confirm('询问', '您是否要永久删除当前数据？'+tip, function(b) {
            if (b) {
                progressLoad();
                $.post('${path}/photographer/deleteForever', {
                    ids : id
                }, function(result) {
                    if (result.success) {
                        parent.$.messager.alert('提示', result.msg, 'info');
...
<div id="photographerToolbar" style="display: none;">
    ...
    <shiro:hasPermission name="/photographer/delete">
        <a onclick="photographerDeleteFun();" href="javascript:void(0);" class="easyui-linkbutton" data-options="plain:true,iconCls:'glyphicon-remove  icon-yellow'">隐藏</a>
        <shiro:hasRole name="超级管理员">
            <a onclick="photographerDeleteForever()" href="javascript:void(0);" class="easyui-linkbutton" data-options="plain:true,iconCls:'glyphicon-remove  icon-red'">永久删除</a>
        </shiro:hasRole>
    </shiro:hasPermission>
    <shiro:hasPermission name="/photographer/add">
