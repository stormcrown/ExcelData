...
CREATE TABLE `video_cost`  (
  `id` bigint(19) NOT NULL AUTO_INCREMENT COMMENT '主键id',
  `update_time` datetime NULL DEFAULT CURRENT_TIMESTAMP COMMENT '更新时间',
  `updated_by` bigint(19) NOT NULL COMMENT '最后更新者',
  `create_time` datetime NOT NULL COMMENT '创建时间',
  `created_by` bigint(19) NOT NULL COMMENT '创建者',
  `delete_flag` int(1) NULL DEFAULT 0 COMMENT '是否删除',
...
#   `cumulative_consumption_ranking` int(11) NULL DEFAULT NULL COMMENT '消耗累积排名',
  ...
  INDEX `video_cost_created_FK`(`created_by`) USING BTREE,
  INDEX `video_cost_updated_FK`(`updated_by`) USING BTREE,
